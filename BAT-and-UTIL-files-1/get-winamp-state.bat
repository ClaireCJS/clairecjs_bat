...
rem ::::: FETCH WINAMP STATE IF APPLICABLE:
    if %ISRUNNING eq 0 (goto :IsNotRunning)
        rem ::OLD METHOD relies on unreliable LWP::Simple: set MUSICSTATE=%@EXECSTR[winamp-status-from-file.pl]
        rem ::NEW METHOD relies on external wget:
...
rem ::::: COPY THE STATE TO ALIAS VARIABLES:
    set  MUSIC_STATE=%MUSICSTATE%
    set WINAMP_STATE=%MUSICSTATE%

rem ::::: EMOJIFY & COLOR-CODE THE RESPONSES:
...
rem ::::: REPORT THE STATE (if not in silent mode):
