...
rem MESSAGE: Message extension parameters because our filemask env vars are usually "*.txt;*.lst" and we need "txt lst"
        set  VALID_EXTENSIONS=%@REPLACE[*.,,%@REPLACE[;, ,%EXTENSION_LIST%]]
        REM echo VALID_EXTENSIONS is “%VALID_EXTENSIONS%”
...
rem Strip “.maybe” post-extension we use in some of our systems:
        if "%VALIDATION_FILE_EXT%" == "maybe" set VALIDATION_FILE_EXT=%@UNQUOTE[%@EXT[%@NAME[%VALIDATION_FILE]]]

rem Check our extension versus the list of valid extensions:
        for %tmp_file_extension in (%VALID_EXTENSIONS) do (
                if %DEBUG gt 0 (call debug "- checking if extension “%tmp_file_extension%” applies to file “%VALIDATION_FILE%” which has VALIDATION_FILE_EXT=“%VALIDATION_FILE_EXT”-- test:if “%@EXT[%VALIDATION_FILE]” == “%tmp_file_extension” goto :Validated_File_Extension_Successfully")
                if "%VALIDATION_FILE_EXT" == "%@UNQUOTE[%tmp_file_extension]" (
...
rem ERROR: At this point, all checks have failed and the file is not valid!
        set VAL_FILE_EXT_ERR_MSG=*** Validation of file “%VALIDATION_FILE%” failed because its extension is not one of: “%italics%%underline%%VALID_EXTENSIONS%%italics_off%%underline_off%”
