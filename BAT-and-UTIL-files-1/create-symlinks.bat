...
::::: PARAMETERS:
    if "%@UPPER[%1]" == "TEST" goto :Test
    if "%@UPPER[%2]" == "TEST" goto :Test

::::: PRE-VALIDATION:
    call checkmappings nopause
    if "%ALREADY_VALIDATED_CREATE_SYMLINKS_PREVAL%" == "1" goto :NoPreVal
...
::::: MANAGE ALL OF OUR SYMBOLIC LINKS: SETUP: 
    ...
    if "%OS%" == "2K" .or. "%OS%" == "XP" quit
    if "%@UPPER[%1]" == "BYLETTERONLY" (gosub :CreateAutomaticSymlinks %+ goto :END)
...
:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    ...
    :CreateLink [LINK_SOURCE LINK_TARGET_VARNAME LINK_TARGET_DRIVE_VARNAME ADDITIONAL_JUNCTION_1]
        ...
        ::::: TEST ASSUMPTIONS AND DEREFERENCE PARAMETERS (optimized harder-to-read order):
            if "%LINK_SOURCE%"       == "" (call alarm-beep FATAL ERROR: CreateLink cannot be called with an empty LINK_TARGET       %+ quit)
            set LINK_TARGET=%[%LINK_TARGET_VARNAME%]             %+ if "%DEBUG%" == "1" echo [DEBUG] LINK_TARGET       for %LINK_TARGET_VARNAME%       is %LINK_TARGET%
            set LINK_TARGET_DRIVE=%[%LINK_TARGET_DRIVE_VARNAME%] %+ if "%DEBUG%" == "1" echo [DEBUG] LINK_TARGET_DRIVE for %LINK_TARGET_DRIVE_VARNAME% is %LINK_TARGET_DRIVE%
            if "%LINK_TARGET%"       == "" (call alarm-beep FATAL ERROR: CreateLink cannot be called with an empty LINK_TARGET       %+ quit)
            if "%LINK_TARGET_DRIVE%" == "" (call alarm-beep FATAL ERROR: CreateLink cannot be called with an empty LINK_TARGET_DRIVE %+ quit)

        ::::: IS THE DRIVE EVEN READY?
            set READY=%@READY[%LINK_TARGET_DRIVE%]                          %+ rem  If the drive isn't ready, nothing we can do about it.
            if "%READY%" == "0" set COLOR=red
            if "%READY%" == "1" set COLOR=green 
            :DEBUG: echo ready=%READY%, color=%color%

        ::::: LET USER KNOW WHAT WE'RE DOING.
                                             echo.
            *color         yellow on black %+ echos *** %+ if "%READY%" == "0" (color bright %COLOR% on black %+ echos  NOT)
...
:DEBUG      if isdir         %LINK_DIR%     echo  mklink /d %LINK_SOURCE% "%LINK_TARGET%" %+ rem  And then create the link
        ...
        ::::: CREATE ADDITIONAL JUNCTION IF PASSED:
            if    ""   ==    "%ADDITIONAL_JUNCTION_1%" return
    ...
    :RemoveBadSimlinks [link_source link_target LINK_TARGET_VARNAME additional_junction_1]
        ...
        :: if we are only checking for one link, and it doesn't exist, we can't possibly remove it:
            if not isdir %link_source% .and. "" == "%ADDITIONAL_JUNCTION_1%" (if "%DEBUG%" == "1" echo [debug] reason 1 %+ return)

        :: if we are only checking for more one link, and none of them exist, we can't possibly remove anything:
            if not isdir %link_source% .and. not isdir %ADDITIONAL_JUNCTION_1% (if "%DEBUG%" == "1" echo [debug] reason 2 %+ return)
        ...
        :: if the proper target we want is not where the primary junction is currently pointing, then it's an expired location. Remove it.
            if "%@UPPER[%@TRUENAME[%link_target%]]" != "%@UPPER[%@TRUENAME[%link_source%]]" (if "%DEBUG%" == "1" echo [debug] reason 3 %+ goto :Remove_YES)

        :: if there are no additional junctions to evaluate, everything is good! We're done!
            if "%ADDITIONAL_JUNCTION_1" == "" (if "%DEBUG%" == "1" echo [debug] reason4 %+ return)

        :: if there ARE additional junctions to evaluate, we need to see if those match the target as well:
            if "%@UPPER[%@TRUENAME[%link_target%]]" != "%@UPPER[%@TRUENAME[%@TRUENAME[%additional_junction_1%]]]" (if "%DEBUG%" == "1" echo [debug] reason 5 %+ goto :Remove_YES)
        ...
        :: but if anything went wrong, we'd be directed here:
            ...
            if "%additional_junction_1%" != "" .and. isdir  %additional_junction_1% (rd %additional_junction_1%)
	...
	:MapSomethingByLetter [letter basedrive base whattolookfor]
        ...
        :: Check if drive ready
            if "%@READY[%letter%]" != "1"  (color bright red on black %+ echo * Drive %letter%: NOT ready... %+ color bright green on black %+ goto :MapSomethingByLetterReturn)
        ...
        :: Don't link to self! That will create inifinte loops!
            ...
            :: 3) Say we are building c:\%whattolookfor%\ on this computer - we can't create a link to P: if P: maps to C:  and is actually C:, or it will become infinite
                for %computer in (%ALL_COMPUTERS%) if "%@UPPER[%[%letter]]" == "%@UPPER[%[%[DRIVE_C_%computer%]]]" (set SAME=%computer% %+ goto :SameDriveAnotherComputer_YES)
                if "%@UPPER[%BASEDRIVE%]" ==     "%@UPPER[%letter%]"   goto :SameDrive_YES
                if "%@UPPER[%BASEDRIVE%]" == "%[%[%@UPPER[%letter%]]]" goto :SameDrive_YES
...
:::
:::        :: Check if drive ready
:::            if "%@READY[%letter%]" != "1"  (color bright red on black %+ echo * Drive %letter% is NOT ready... %+ color bright green on :::black %+ goto :MapMediaByLetterReturn)
:::            color bright green on black %+ echos * Drive %letter% is ready...
:::
:::        :: Don't link to self! That will create inifinte loops!
