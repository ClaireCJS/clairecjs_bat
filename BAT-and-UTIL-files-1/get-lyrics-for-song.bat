...
rem CONFIG: PROBER:
        ...
        set DEBUG_DISPLAY_PROBE_RESULTS=0                             %+ rem whether we want to display the probed values to the screen for debug purposes
...
rem VALIDATE ENVIRONMENT [once per session]:
        iff 1 ne %VALIDATED_GLVMS_ENV then
                call validate-in-path              %LYRIC_DOWNLOADER_1% %PROBER% delete-zero-byte-files get-lyrics-with-lyricsgenius-json-processor.pl tail echos  divider unimportant success alarm unimportant debug warning error fatal_error advice  important important_less celebrate eset eset.bat eset-alias.bat insert-before-each-line.pl insert-before-each-line.py pause-alias google.bat google.py google.pl insert-before-each-line.py newspaper.bat print_with_columns.py print-with-columns.bat newspaper.bat srt2lrc.py change-single-quotes-to-double-apostrophes.py add-ADS-tag-to-file.bat remove-ADS-tag-from-file.bat display-ADS-tag-from-file.bat display-ADS-tag-from-file.bat approve-lyrics.bat disapprove-lyrics.bat display-lyric-status-for-file.bat review-lyrics.bat review-files.bat approve-lyric-file.bat disapprove-lyric-file.bat approve-subtitle-file.bat disapprove-subtitle-file.bat get-lyric-status.bat
...
rem VALIDATE PARAMETERS [every time]:
        ...
        set  AUDIO_FILE_BASE=%NAME[%AUDIO_FILE%]
        set  AUDIO_FILE_NAME=%NAME[%AUDIO_FILE%]
        set  AUDIO_FILE_EXT=%EXT[%AUDIO_FILE%]
...
call divider
call bigecho "%STAR% Getting lyrics for '%faint_on%%italics_on%%audio_file%%italics_off%%faint_off%'"
...
rem Debug info:
        ...
        if 1 eq %DEBUG_DISPLAY_PROBE_RESULTS% call debug "Retrieved:%TAB%   artist=%FILE_ARTIST%%TAB%%tab%%tab%%tab%        title=%FILE_SONG%%TAB%%tab%%tab%%tab%        album=%FILE_ALBUM%%TAB%%tab%%tab%%tab%  orig artist=%FILE_ORIG_ARTIST%"
...
rem Check if we already have a TXT file in the same folder and shouldn't even be running this:
        iff exist "%LYRIC_FILE%" .and. %@FILESIZE["%LYRIC_FILE%"] gt 0 then
                ...
                rem Display lyrics:
                        ...
                        (type "%LYRIC_FILE%" |:u8 unique-lines -A -L) |:u8 print-with-columns
                        @call divider
                ...
                rem If the lyrics are good, set them as so. if they are not, warn:                        
                        iff "%ANSWER%" eq "Y" then                        
                                ...
                                if exist "%LYRIC_FILE%" (ren  /q "%LYRIC_FILE%" "%LYRIC_FILE%.%_datetime.bak")
                                rem  divider
...
rem Check if we have one in our lyric repository already, via 2 different filenames, and then manual selection:
        ...
        iff exist "%MAYBE_LYRICS_1%" .and. %@FILESIZE["%MAYBE_LYRICS_1%"] gt 0 then
                ...
                iff "%ANSWER%" eq "Y" then
                        *copy "%MAYBE_LYRICS_1%" "%LYRIC_FILE%" >nul
                ...
                iff "%ANSWER%" eq "Y" then
                        *copy /q "%TMPREVIEWFILE%" "%LYRIC_FILE%" >nul
                        iff not exist "%LYRIC_FILE%" then
                                call error "LYRIC_FILE of %LYRIC_FILE% doesn't exist and should"
                        endiff
                        set LYRICS_ACCEPTABLE=1
                        goto :have_acceptable_lyrics_now_or_at_the_very_least_are_done
                else
...
rem Download the lyrics using LYRIC_DOWNLOADER_1: BEGIN: ————————————————————————————————————————————————————————————————————————————————————
        ...
        rem See if our latest file is the expected extension [which would indicate download sucess] or not:              
                ...
                iff "%@EXT[%LATEST_FILE]" == "%LYRIC_DOWNLOADER_1_EXPECTED_EXT%" then
                        ...
                        *ren /q "%LATEST_FILE%" "%PREFERRED_LATEST_FILE_NAME%" >nul
                        ...
                        @echos             %@ANSI_MOVE_TO_COL[0]               %@ANSI_MOVE_TO_COL[0]``
                        call divider
                        @call warning "%conceal_on%(A)%conceal_off% No lyrics downloaded %conceal_on%(A)%conceal_off%" silent
        ...
        rem We are about to make a TXT file. If it exists, better back it up first:
                if exist "%LYRIC_FILE%" (ren /q "%LYRIC_FILE" "%LYRIC_FILE%.%_datetime.bak">nul)
        ...
        rem At this point, our SONG.txt should exist!  If it doesn't, then we rejected all our downloads.
                iff not exist "%LYRIC_FILE%" .or. %@FILESIZE["%LYRIC_FILE%"] eq 0 then
                        @echos             ``
                        call divider
                        @call warning "%conceal_on%(B)%conceal_off% No lyrics downloaded. %conceal_on%(B)%conceal_off%"
                        rem set LYRIC_RETRIEVAL_1_FAILED=1
                else
                        @call divider
                        ...
                        iff "%ANSWER%" eq "Y" then
                                ...
                                ren  /q "%LYRIC_FILE%" "%LYRIC_FILE%.%_datetime.bak"
                        endiff
                endiff
...
rem Mark the lyric file as approved/disapproved, using windows Alternate Data Streams:
        ...
        iff exist "%LYRIC_FILE%" .and. 1 ne %LYRICS_FOUND_TO_HAVE_BEEN_PREAPPROVED then
                call divider
                if 1 eq %LYRICS_ACCEPTABLE call    approve-lyric-file "%LYRIC_FILE%"
                if 0 eq %LYRICS_ACCEPTABLE call disapprove-lyric-file "%LYRIC_FILE%"
        endiff
...
rem Validate we did something:
        ...
        iff not exist "%LYRIC_FILE%" then
                ...
                call celebrate "%ansi_background_black% %check% %@cursive[LYRIC SUCCESS] %check%  %@randfg[]" 
                call important_less "Lyrics downloaded: %blink_on%%italics_on%%LYRIC_FILE%%ANSI_RESET%"
