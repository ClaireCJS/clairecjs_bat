...
REM embed the art into the song, then capture the errorleve:
        echo %italics%eyed3.exe --add-image="%@UNQUOTE[%ART]:FRONT_COVER" %MUSIC% %italics_off%
                      eyed3.exe --add-image="%@UNQUOTE[%ART]:FRONT_COVER" %MUSIC% 
        set EYE3DERRORLEVEL=%_?
        REM %COLOR_UNIMPORTANT% %+ echo EYE3DERRORLEVEL  is %EYE3DERRORLEVEL%  %+ %COLOR_NORMAL


REM react to any captured errorlevel:
...
REM continue dealing with any errorlevel:
        ...
        if ERRORLEVEL 0 (%COLOR_SUCCESS% %+ echo %EMOJI_CHECK_MARK% MP3 cover art embed success!!! %+ if %DONT_DELETE_ART_AFTER_EMBEDDING ne 1 (%COLOR_REMOVAL %+ del %ART%))
