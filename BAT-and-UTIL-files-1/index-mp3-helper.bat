...
:DESCRIPTION: ::::::::::::::::::::::::::::::rem INDEX DIGITAL AUDIO ::::::::::::::::::::::::::::::::::::
...
:DEFINE_VARIABLES
	...
	if "%USERNAME"=="%CAROLYN" SET NUM_RECENT_TO_KEEP=500
        set RECENTLISTFILE=%RECENT_FILE_LIST%


:PRE_SETUP
        timer>nul
        rem  Validate the environment EVERY time:
                call     validate-environment-variables  MP3OFFICIALDRIVE MP3LISTDRIVE MP3OFFICIAL EDITOR MP3LIST MP3LISTBAK NUM_RECENT_TO_KEEP RECENTLISTFILE BAT COLOR_RUN COLOR_WARNING username color_debug color_important color_normal
                call     ensure-drive-is-mapped         %MP3OFFICIALDRIVE%
                call     validate-in-path                mchk mp3index generate-audio-playlists generate-filelists-by-attribute.pl generate_thesem3u_and_allm3u_playlists_in_folder.py diff pushd du success print-message.bat wcheader wc  mchk.bat generate-audio-playlists.bat tail winsort sort important  insert-before-each-line  grep
                call     checkusername
        cls
        timer /3 on
...
:BACKUP_CERTAIN_FILES
        %COLOR_REMOVAL%
        if     exist "%MP3LIST%"    (echo ray|*copy /q "%MP3LIST%" c:\recycled\latest-everything.m3u) %+ echos                 >:u8   "%MP3LISTBAK"     %+ REM Let's try just doing this always —— an indentation
        ...
        rem DEBUG: pause "Backup certain files done"
...
:ACTUALLY_RUN_THE_INDEXER_SUBSCRIPT:
        ...
        rem open results in text editor
                pushd
                        %MP3OFFICIAL%\LISTS\
...
:POST_INDEX_TRIGGERS_TO_LET_REACTIVE_PROCESSES_KNOW_THIS_HAS_HAPPENED
	>"%MP3OFFICIAL\LISTS        RIGGER-CASMP3.TRG"
	>"%MP3OFFICIAL\LISTS        RIGGER-CL-4G-SD-CARD.TRG"
	>"%MP3OFFICIAL\LISTS        RIGGER-CL-IPHONE2G.TRG"

:THINGS_WE_DO_NOT_DO_ANYMORE
	:SYNC_PLAYLISTS_TO_PORTABLE_DEVICES
		rem :::: We may get stuck here, so let's start it in a new window until we master it.
		rem :::: NO! THIS IS NOW OFFLOADED TO CAROLYN'S 4NT WINDOWS! AWESOME! call sync-mp3-playlists-to-carolyn's-mp3player.bat
		rem :::: 2014 update: We now have a nice sync.bat that we manually run when inserting memory cards into cardreaders and such
	:REMAP_PLAYLISTS_TO_CORRECT_DRIVE_LETTERS
		rem ::::	NO LONGER DOING THIS, as I've decided all playlists should be c:\mp3\, and computers can create junctions at c:\mp3\ that point to wherever the music actually is
		rem ::::	This has caused a half-crash on Hades due to Hell's networking problems:
		rem ::::	call remap-playlists.bat
	:RELOAD_PLAYLISTS
		rem :::: rem Reload playlists for secondary and other music servers, so new playlists are instantly reflcted:
		rem :::: 	call reload-playlists.bat
		rem :::: echo TODO: Note that reloading the playlist like that does NOT instantly reflect changes anymore, due to changes in how they are reloaded!
		rem :::: echo       Nowadays we just reload now-playing-playlist.m3u which does NOT get updated!
		rem :::: echo       What we could do is look in now-playing.dat and simply copy that file over now-playing-playlist.m3u
		rem :::: echo             - but that will not work with "these.m3u" and "all.m3u" style lists, just meta-attribute lists!
...
::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
:CREATE_RECENTLIST
	rem sET TMP1=c:\recycled\latest-diff.txt
	rem sET TMP2=%@UNIQUE[c:\recycled]
	rem sET TMP3=%@UNIQUE[c:\recycled]
	...
	:201503 ADDITION:
        ...
        rem Edit it if it exists:
                if exist %TEMP\changer-diff-report.txt (%EDITOR %TEMP\changer-diff-report.txt)


	rem Let's see what's gone into the changer/not into it
	...
	rem Let's see what's gone into the party/not into it
	...
	rem Let's see what's gone into the best/not into it
	...
	rem Might be good to edit the file to remove extraneous entries, but we decided to stop doing this:
...
:END
    ...
    rem In the past, the file exceeded the maximum file size for older versions of EditPlus, so we would check for presence of an alternate text editor (SlickEdit):
        IF NOT DEFINED BIGEDITOR set BIGEDITOR="%Program Files%\SlickEdit Pro 21.0.0\win\vs.exe"
