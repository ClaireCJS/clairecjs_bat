...
REM Behavior overides and message decorators depending on the type of message?
    ...
    if  "%TYPE%"  eq "IMPORTANT"      (set DECORATOR_LEFT=%EMOJI_TRUMPET%%EMOJI_TRUMPET%%EMOJI_TRUMPET%  %ANSI_RESET%%@ANSI_FG_RGB[76,0,76]%reverse_on%%blink_on%%EMOJI_FLEUR_DE_LIS%%blink_off%%reverse_off%%ANSI_COLOR_IMPORTANT% `` %+ set DECORATOR_RIGHT= %ANSI_RESET%%@ANSI_FG_RGB[76,0,76]%reverse_on%%blink_on%%EMOJI_FLEUR_DE_LIS%%blink_off%%reverse_off%%ANSI_COLOR_IMPORTANT%)
