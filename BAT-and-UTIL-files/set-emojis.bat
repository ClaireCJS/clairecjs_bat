...
setdos /X-7 
    ...
    REM 1: DECDLD seq to define custom charset (aka soft font) w/four characters: "()*+" [ascii 40-44] representing the trumpet glyphs.
    REM 2: To activate charset, use SCS sequence w/id "@", i.e. "\x1B( @" [note that there's a space before the @!]
    REM    Then you can output left trumpet w/ *( & right w/ )+.
    REM    To return to regular text, you MUST switch back to charset with SCS sequence "\x1B(B"

        REM we used to do this with cygwin, then we learned to do it internally:
            rem if not exist c:\cygwin\bin\printf.exe (call error "we need cygwin's printf for this. TCC's didn't work tho it wasn't fully investigated.")
            ...
            REM  %ESCAPE%P0;8;1;10;0;2;20;0{ @???~}{wo_?/[MFJp@@@@@/GCA@??????/??????????;?_ow{}~???/@@@@@pJFM[/??????@ACG/??????????;??????????/??_ogKYr}{/?_ow\\NVb`O/@B@???????;??????????/{}rYKgo_??/O`bVN\\wo_?/???????@B@;?_OGCCAaYM/}BEIQayFAA/BKO_oM`PIE/____@@AAAA@B@;YaACCGO_??/AFyaQIEB}?/IP`Mo_OKB?/AAA@@?????%ESCAPE% 
...
REM Flag that the emoji have been defined so we don't do this over and over:
