...
rem Validate environment, but only once:
        if %validated_reset_tab_stops ne 1 .or. "%3" eq "validate" (
            call validate-env-vars  ANSI_TABSTOP_SET_COL OUR_TAB_WIDTH_TO_USE NUM_TABS_TO_REGEN ANSI_POSITION_SAVE ANSI_POSITION_RESTORE ANSI_TABSTOP_CLR_COL ANSI_TABSTOP_SET_COL STAR BLINK_ON BLINK_OFF ANSI_COLOR_SUCCESS PARTY_POPPER DEFAULT_TAB_STOP_WIDTH
...
rem (Hidden Cosmetic Voodoo)
        echo.
        echos %ANSI_POSITION_RESTORE%%ANSI_MOVE_UP0%%ANSI_COLOR_SUCCESS%%PARTY_POPPER DONE!%ANSI_CLEAR_TO_EOL%
        echos %@ANSI_MOVE_TO_COL[1]%ANSI_CLEAR_TO_EOL%
