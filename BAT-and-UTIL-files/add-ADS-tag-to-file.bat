...
rem Validate environment and parameter:
        ...
        echo on
        call validate-environment-variable  File_To_Use    "1ˢᵗ arg must be a filename. 2nd optional arg must be a tag, 3rd arg must be 'read' or a value, 4ᵗʰ optional arg can be 'verbose'"
        echo off
        call validate-environment-variable   Tag_To_Modify "2ⁿᵈ argument must a tag, NOT empty"
        call validate-environment-variable   Tag_Value     "3ʳᵈ argument must a value, or 'read' ... NOT empty"
        if "%PARAM_4%" ne "" .and. "%PARAM_4%" ne "verbose" .and. "%PARAM_4%" ne "remove" (call    fatal_error    "There shouldn't be a 4ᵗʰ parameter of this value being sent to %0 {called by %_PBATCHNAME}, but you gave '%italics_on%%PARAM_4%%italics_off%'. Run without parameters to understand proper usage.")
...
rem Determine verb clause to use —— looks best if they are >10 chars each though!
        ...
        set SPACER=%@REPEAT[ ,%@ABS[%@EVAL[%@LEN[%@STRIPANSI[%VERB%]]-10]]]``
...
rem Read or set (depending on invocation) via windows alternate data streams:
        ...
        elseiff "remove" eq "%TAG_VALUE%" then
                ...
                rem If we are in verbose mode, verify the write by reading it:
                        ...
                        :        call %0 "%FILE_TO_USE%" %TAG_TO_MODIFY% read verbose "%VERB%"
                        :endiff                                
                        gosub explain
        endiff
        ...
        :explain
                rem If we are in verbose mode, explain what we did:
                        iff 1 eq %VERBOSE then
                                ...
                                echo %SPACER%%ansi_color_green%%normal_arrow%%ansi_color_normal%  %faint_on%from%faint_off% file: %@ANSI_FG_RGB[168,210,104]%[FILE_TO_USE]%ansi_color_normal%
                        endiff                                
        return
