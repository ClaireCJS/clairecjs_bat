...
rem ENVIRONMENT VALIDATION (once):
        iff "1" != "%validated_put%" then
                call validate-environment-variables ansi_has_been_set emoji_have_been_set
                call validate-in-path fixclip print-message warning grab-helper.pl echos copy-move-post rmdir highlight
                set  validated_put=1
        endiff
	...
	if     exist  %1  goto :isfile
        :undefined
            %COLOR_WARNING% 
                ...
                echo So far, this only works on folders. So %lq%%1%rq% would have to be a folder.
		goto :END
        :NoArg2
...
rem ENSURE TARGET DOESN'T ALREADY EXIST:
...
rem MOVE IT:
        ...
        %COLOR_DEBUG% %+ repeat 2 echo. %+ echo %STAR% Ready to: %REDOCOMMAND% %+ %COLOR_NORMAL%
        if "%CONFIRM%" eq "1" .or. "%DEBUG%" eq "1" (%COLOR_IMPORTANT% %+ pause)
        %COLOR_SUCCESS%
        (echo y | %REDOCOMMAND%) |&:u8 copy-move-post 
        %COLOR_NORMAL%

rem ENSURE ORIGINAL SOURCE IS GONE, IF NOT, TRY AGAIN AND/OR WARN:
	...
	if isdir %SOURCE call warning "%1 still exists %DASH% %emphasis%rmdir '%1'%deemphasis% %BLINK_ON%failed%BLINK_OFF%!"
	goto :END
